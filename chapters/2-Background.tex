...
For an illustrative example, consider the very simple parallel program to compute the sum of a set of integers $\{1,\ldots,\,10\}$ using two threads, $\Pi_1$ and $\Pi_2$. Each thread will then load a subset of the work to be done; perhaps $\Pi_1$ calculates that the sum of $\{1,\ldots,\,5}$, while simultaneously, $\Pi_2$ calculates that the sum of the rest. A naive approach would be to simply create a new variable $s$, into which the threads will accumulate the values.  LANG pg 25


%
\begin{equation}
	\bF := \left \{1,\ldots,\,10\right \}
...
\nl		$\ProcSty{unlock}(\mu_a)$\;
	...
	\caption{Parallel algorithm for building the family of sets of all members of each neighborhood discovered in the mesh \label{alg:critialSection}}
\end{algorithm}%
