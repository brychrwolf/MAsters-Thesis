...
\todoReword{add variant oninterpolation for NOT between points}
...
\end{equation}
To illustrate the concept, consider the two points $(1, 2)$ and $(5, 1)$, then find the interpolant of a point located at $x = 3$\todoReword{interpolant is wrong, don't use point located at, maybe value sampled at instead?}
\begin{align}
...
\nomenclature[]{$A$}{an area of a circular sector}%
...
\end{equation}%
\nomenclature[]{$\check{\ell}$}{the distance from $\bp_0$ to $\bc$}%
...
\nomenclature[]{"bisecting line"}{the line which bisects the center angle of a circle sector}
