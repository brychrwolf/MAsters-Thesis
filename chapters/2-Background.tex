...
In modern software, the square root operation is performed by computing multiple iterations of the so-called recurrence equation using Newton's method\todoCitation{software uses Newton's iteration for sqrt}, otherwise known as "Newton's Iteration." ~\cite{Weisstein19b} The impact for \forf{t} is that the computation the square root typically\todoCitation{how slow is Newton's iteration compaed to others} takes many more compute cycles than other operations, and thus takes more time to complete. In fact, because of the slowness of the square root operation, computing the L2-norm in order to calculate an edge's length is emperically the most costly operation performed by the filter. \todoResearch{qualify, do experiment to prove how slow sqrt is}, Therefore, it is imperitive that one takes special attention to avoid unneccessary calls to calculate an edge's length, so we define the symbol $|\xi|_\star$ to facilitate designing an efficient implementation of \forf{t}.
%
\begin{align}
%\begin{equation}
%\begin{aligned}
...
\nomenclature[c2a]{$\bM$}{a mesh; the superset which includes the sets of all points $\bp$ and faces $\bt$}%
