...
%
%
\section{Basic Theory}\todoReword{theorhetical foundation}
...
\end{equation}%
\nomenclature[c2b]{$\bP$}{the set of points $\bp$ in $\bM$}%
\nomenclature[bd]{$\bp$}{a point}%
\nomenclature[be]{$\bp_v$}{a specific point in $\bP$}%
...
\nomenclature[bf]{$\bp_k$}{a specific point in $\bN$}%
\nomenclature[bg]{$\bp_i$}{also $\bp_{\sipo}$, and $\bp_{\sipt}$; an indirectly referenced point within a particular face}%
\nomenclature[bh]{$\bp_0$}{also $\bp_1$, and $\bp_2$; a directly referenced point within a particular face}~\cite[p.~25]{Mara12}%
...
\end{equation}%
\nomenclature[c2c]{$\bT$}{the set of faces $\bt$ in $\bM$}%
\nomenclature[bd]{$\bt$}{a face}%
\nomenclature[be]{$\bt_f$}{a specific face in $\bT$}%
\todoAsk{is own cardinality in own definition okay?}
...
\end{align}
%
In modern software, the square root operation is performed by computing multiple iterations of the so-called recurrence equation using Newton's method\todoCitation{software uses Newton's iteration for sqrt}, otherwise known as "Newton's Iteration." ~\cite{Weisstein19b} The impact for \forf{t} is that the computation of a square root typically\todoCitation{how slow is Newton's iteration compaed to others} takes many more compute cycles than other operations, and thus takes more time to complete. In fact, because of the slowness of the square root operation, computing the L2-norm in order to calculate an edge's length is emperically the most costly operation performed by the filter. \todoResearch{qualify, do experiment to prove how slow sqrt is} Therefore, it is imperitive that we takes special attention to avoid unneccessary calls to calculate an edge's length, so we define the symbol $\xistar$ to facilitate that attention in designing an efficient implementation of \forf{t}.
%
...
Converserely, synthetic \tdd{} is artfully crafted to obey the contraints of being a good mesh\todoReword{define a good mesh here}, without the complexities exhibited by acquired data. When modeling 3-dimensional objects, synthetics can require significantly less memory for storage, as simplifications can made for large regular surfaces, for example, even the largest rectangular surface can be modeled with only four points and two faces, whereas the acquired methods require that the Nyquist-Shannon sampling theorem be obeyed for the smallest detectable feature throughout the entire surface.\todoCitation{Nyquist Shannon Theorem}
