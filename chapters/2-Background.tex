...
%
\section{\tdd}
The data upon which one convolves \forf{} is called \tdd\todoCitation{\tdd name origin, BB82 from Mara12}. It consists primarily of two elements, points $\bp$ and faces $\bt$, which when taken together, comprise the mesh $\bM$. \tdd also comes in two disticnt flavors depending on its origin: acquired or synthetic. The data can as well include a texture map, or other various types of information stored as scalar fields upon the points, with a few examples being: RGB color, material type, reflectivity, transparency, quality, confidence, or the resulting function values from an analytical filter such as the Multi-Scale Integral Invariants (MSII) filter~\cite[p.~21]{Mara12}. \forf{} is agnostic to information represented by the scalar fields, and can similarly process any such data.\todoReword{any maybe too strong, constrained to single field at a time}%
\nomenclature[a0]{$\bM$}{a mesh; the superset which includes the sets of all points $\bp$ and faces $\bt$}%
%
\subsection{Points}
A point $p$ is the most primitive element of \tdd. As the abbreviated form of "measuring point", also known in other contexts as a vertex, and position vector in $\mathbb{R}^3$\todoCitation{other names for a point}, a point is defined by the 3-dimensional Cartesian coordinates $x$, $y$ and $z$. In \tdd, points are generally unique and not required to be in any particular order. In this thesis, a point is addressed using multiple\todoReword{how many?} different subscripts.

For its globally unique index, we first define the set $\bP$, the reference each unique point as $\bp_v$:
\begin{equation}
	%Set version
	\bP = \left \{\bp \mid \forall \bp \in \bM \right \}
	%List version
	%\bP = \left \{\bp_1,\:\ldots,\:\bp_i,\:\ldots,\:\bp_{i_{max}}\right \}\;\text{with}\;i_{max} = |\bP|\;\text{and}\;i = \mathbb{Z}^+ \land i \leq ∧ i_{max}
	\label{eq:defineSetOfPoints}
\end{equation}%
\nomenclature[a0]{$\bP$}{the set of points $\bp$ in $\bM$}%

When referencing a $\bp$ within a contextually specific neighborhood:
\begin{equation}
	\bP = 
	\label{eq:definePointsInNeighborhoods}
\end{equation}%
\nomenclature[a0]{$\bP$}{the set of points $\bp$ in $\bM$}%
%
p0, p1, p2, 
pi, pi+1, pi+2, (pj in alg:3)
pv
pk


address them by the index i and store them as a list L v :
