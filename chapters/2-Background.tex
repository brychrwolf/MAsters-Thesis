...
\todoReword{add variant on interpolation for NOT between points}
...
\nomenclature[]{$\bO_v$}{a specific geodesic disc centered about the point $\bp_v$}%
...
%
and double indices $v,i$, when an edge length is referenced in relation to a specific point $\bp_v$ and its neighbor $\bp_i$
%
\begin{equation}
	\ell_{v,i} := |\bp_i - \bp_v|
	\label{eq:defineEdgeLengthPoint}
\end{equation}%
\nomenclature[ag]{$\ell_{v,i}$}{the length of distance from point $\bp_v$ to $\bp_i$}%
