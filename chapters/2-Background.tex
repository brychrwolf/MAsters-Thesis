...
\label{ch2sBssLA}
Nearly as fundamental as set theory, linear algebra is the  mathematical field which studies the different methods and representations which may concern a line, including equations, matrices, vector arrays,  norming functions, etc. It also provides the symbols and concepts for performing blocks of operations in parallel.\todoCitation{linear algebra}
\todoReword{move references to Euclidean things up here}

%
\subsubsection{Position Vectors}
\label{ch2sBssLAsssPV}
A position vector is represented by a set of Euclidean coordinates with cardinality matching that of the dimensions of the space in which it is embedded.\todoCitation{position vector} For example, in Section~\ref{ch2sBss3sssP}\todoReword{add back reference for this section} we say that a point can also be called a position vector, so if we are given a point in $\mathbb{R}^3$, it is defined by the three Cartesian coordinates $x$, $y$, and $z$.

Figure~\ref{fig:definePositionVector} provides an example of why it is called a vector. Any set of coordinates have an implied direction pointing away from the origin which is located at \{0, 0, 0\}.

\begin{figure}[h]
\ffigbox
	{\includegraphics[width=1.0\linewidth]{figures/definePositionVector.png}}
	{\caption[Example of a Position Vector]{An example of a position vector in $\mathbb{R}^3$. The origin in coral color is located at $\{0, 0\}$. The point $A$ in teal color is located at $\{16, 9\}$. The arrow in sand color pointed from the origin to the point is the reason for the name position vector.}\label{fig:definePositionVector}}
\end{figure}
\todoReword{reason for the name sounds dumb}
...
The equation for calculating the L2-norm of a position vector in $\mathbb{R}^3$ is
\begin{equation}
...
\ffigbox{
	\begin{tikzpicture}[node distance=1.25cm, every node/.style={fill=white, font=\sffamily}, align=center]
		...
		\draw[->] (adds) -- (sqrtAll);
	\end{tikzpicture}}
	{\caption[Data Dependencies in the L2-norm Calculation]{The data dependencies inherent to calculation of the L2-norm of a position vector in $\mathbb{R}^3$, with each distinct operation colored in teal, and data dependencies marked with arrows. The centered block in sand color represents an exclusive-or situation, abbreviated as xor, where one, and only one pair of operations shall execute.}\label{fig:dataDependencyOfL2Norm}}
\end{figure}
