...
\ffigbox
	...
	{\caption[SIMD Architecture]{A simplified impression of a system characterized as SIMD by Flynn's Taxonomy of concurrent architecture. The array of processors each receive the same kernel from a pool of instructions, then load a different portion of the data pool in order to process to combination in parallel.}\label{fig:simdArchitecture}}
\end{figure}\todoCitation{Flynn, 1966 doi:10.1109/TC.1972.5009071.}

The major advantage obtained by implementing software to utilize the architecture of an SIMD system, is the quantifiable speedup gained, as discussed in detail in Section~\ref{xxxxxx}\todoReference{quantifiable speedup}, when repetitive tasks which would otherwise be executed serially in a loop, one after another, instead are computed simultaneously, in parallel, incurring only minor synchronization overhead. For example: any operation performed in linear algebra, such as the subtraction of two high-dimensional arrays, is thus made much more scalable in regards to the time required for the computation to complete as vectors of larger and larger sizes are considered.

This concept can be extended to more complex programs, and indeed, a major portion of the research presented in this thesis is implementing the software to convolve the complex algorithm for \Fors{t}, to be executed for every point in a mesh simultaneously, by utilizing the parallel architecture of SIMD systems.

, then typically requiring synchronization before the results may be further processed
