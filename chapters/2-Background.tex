...
Sections of code containing the instructions that utilize volatile memory are aptly named ``critical sections'', and special care must be taken in their regard, in order to ensure program correctness, despite not being able to determine in what order the operations may occur.
...
Also important is the notion of an algorithm's ``degree of parallelism'', which is the maximum number of operations which can be executed in parallel. Which leads to the derivation of Amdahl's law\todoCitation{Amdahl's Law}, which is given a contant problem size $\hat{n}_{fixed}$,
%
\begin{equation}
