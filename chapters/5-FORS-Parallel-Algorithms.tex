...
%\todoBackground{path of dependence}
...
Initially, the portion of the total work load, hence forth referred to as ``\gls{tStride}'', is calculated in line~\ref{algPBN.stride}. Then each individual processor will be instructed to compute its own stride, equal to the problem size divided by the number of available processors. Here, the unit of work being only the three points of the triangular face $\bt$. 
...
\nl		$\fM \leftarrow \{\mu_1,\,\ldots,\,\mu_{|\bP|}\}$\;\label{algPBN.mutexSet}
...
\nl		\ProgSty{synchronizeThreads()}\;
	...
	\bigskip
\nl	\Kernel{build($\Pi$, $\sigma$, $\fM$, $\bT$)}{\label{algPBN.build}
...
\nl			\ProgSty{$\sim$safeUnion($\fM$, $\bN$, $\bp_c$, $\bp_a$, $\bp_b$)}\;
	...
	\bigskip
\nl	\Kernel{safeUnion($\fM$, $\bN$, $a$, $b$, $c$)}{\label{algPBN.safeUnion}
\nl		$\ProcSty{lock}(\mu_a)$\;
...
%\todoReword{a figure here would be nice}
...
\todoCitation{recursive algorithms, three parts}
...
\footnotetext{It is important to notice in line~\ref{algPRCN.onlyFirstCall}, the distinction between the family of sets of neighborhoods $\bN$ in the first call of \textit{parallelRecursiveCensusNeighborhoods}, and the set of integers accumulating in the subset $\widetilde{\bN}$ in subsequent calls. Also, non-existent neighborhoods should be treated as having a cardinality of zero.}
...
As can be clearly seen in Figure~\ref{fig:neighborhoods}, the cardinality of each individual neighborhood cannot be predicted for irregular, triangle meshes, like those typical of acquired \tdd{}. However, that becomes less important because it is possible to know the total census count of neighbors in all neighborhoods of $\bN$ by actually counting the size of each neighborhood a posteriori, which is done in Algorithm~\ref{alg:parallelRecursiveCensusNeighborhoods}. In the next chapter, we will use $\hat{n}$ to unroll this pair of loops and instruct a calculable number of threads to efficiently process the loop block in parallel.
...
\nl				\ProgSty{$\sim$safeEdgeLengthCalculation($\mu$, $\bE$, $\gelm$, $\bp_v$, $\bp_i$)}\;
	...
	\bigskip
\nl	\Kernel{safeEdgeLengthCalculation($\mu$, $\bE$, $\gelm$, $\bp_v$, $\bp_i$)}{
...
\nl 	\KwRet $\bF'$\;
	...
	\bigskip
\nl	\Kernel{safeAccumulatesGeoDiscMean($\Pi$, $\sigma$, $\bP$, $\bN$, $\bE$, $\gelm$, $\bF$, $\bF'$)}{
\nl		$\check{\sigma} \leftarrow (\Pi-1)\,\sigma+1$\;
\nl		$\hat{\sigma} \leftarrow \Pi\,\sigma$\;
\nl		\For{$\bp_v \in \{\bp_{\check{\sigma}},\ldots,\,\bp_{\hat{\sigma}}\}$}{
