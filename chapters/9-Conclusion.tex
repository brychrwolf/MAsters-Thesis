...
Implement in OpenCL to include all GPUs
Implement in OpenMP to use multiple machines
Implement in PThreads to exploit MIMP
Pipelining memory reads/calculations exploit more concurrency
Edge case handling
	max mesh size in memory
	Derive calculation for compute time per iteration by mesh size. Maybe find when load time is greater than iteration time
support other file types
Calculating edge length takes longest, so DO NOT DOUBLE EFFORT HERE
...
More analysis on \forf. it is my intuition that the un-isotropic nature of the filter is due to the speed at which information travels along longer edges.
