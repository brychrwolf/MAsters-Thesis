...
For the serial algorithm, we used the implementation existing in the GigaMesh framework~\cite{Mara10}, for the parallel algorithm, a new implementation in CUDA extended C++~\cite[p.~19-66]{CUDA18} was developed and used. Verification of final resulting scalar field of \wmfv{s} were conducted at the conclusion of each pair of experiments, and it was determined that zero difference existed between the output of the serial algorithm versus the parallel variant, even with the largest meshes for the most convolutions.
