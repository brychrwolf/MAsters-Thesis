...
Then, we systematically executed every combination\footnote{Of the 320 possible combinations of experiments, we were able to record data for 277 different combinations, however, it is unclear that any additional data would influence the result of the combined experiments.} of hardware and mesh for eight increasing amounts of convolutions, \{1, 3, 10, 30, 100, 300, 1000, 3000\}, restarting the count for each experiment in order to ensure that the resulting time was genuine.

%Computing, the average timing for each group of experiments resulted in ... the findings. in order to establish the performance of both the serial and parallel variations of the algorithm for \Fors{t}

Figure~\ref{fig:computeTimesLP} shows the timing results of every recorded experiment. The graph is drawn extra tall to ensure that the grid remains square for an unbiased assessment of the correlations therein. Results from each combination of hardware and mesh are connected by lines. Combinations involving the smallest mesh sizes are colored red, and the largest mesh sizes in blue, with all others scaled in-between.
\todoResearch{N.add formula for timing (or at least ref to it) here.}
\todoResearch{N.process time noise at very fast speeds}

\begin{figure}[ht]
\includegraphics[width=1.0\linewidth,height=1.0\textheight,keepaspectratio]
	{figures/computeTimesLinespoints.png}
	\caption[Compute Times - Linespoints]{Compute Times of Applying \fors{t} for Selected Numbers of Iterations onto Acquired and Synthetic 3D Meshes of Varying Sizes

\vspace*{\baselineskip}
\scriptsize† The codes used correspond to the machine T for the desktop or M for the laptop, the algorithm variant P for parallel, or S for serial, a period, a two character code representing the mesh being processed: BS for bisected square, HT for hexagonal tessellation, US for university seal, and FS for the flat surface, then another period, and finally, the count of points comprising the mesh.}
...
Figure~\ref{fig:computeTimesS} graphs the total compute times of each experiment as the area of a circle, given different hardware configurations, for each combination of mesh size, measured in point counts, and the number of convolutions of \fors{t}. In general, one can see that for all experiments, the total compute times increase for with the size of the mesh, as well as the number on convolutions, so that an increase in both results in an exponential increase in the compute time. Which is not surprising given the complexity of the algorithm as discussed in Chapter~\ref{ch3}. One can also see the positive effect of the parallel processing, as the growth of the compute times is much slower for the parallel variant than for the serial.

\begin{figure}[ht]
	\includegraphics[width=\linewidth]{figures/computeTimesScatter.png}
	\caption[Compute Times - Scatter]{A graph of the total compute times of each experiment as the area of a circle, given different hardware configurations, for each combination of mesh size, measured in point counts, and the number of convolutions of \fors{t}. Blue circles indicate the serial variant computed with a CPU, and a red circle indicating the parallel variant was computed on a GPGPU.}
