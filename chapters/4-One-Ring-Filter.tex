...
\nomenclature[a05]{$m$}{the cardinality of $\bM$}%
\nomenclature[a06]{$\bN$}{a one-ring neighborhood}%
\nomenclature[a07]{$n$}{the cardinality of $\bN$}%
...
\end{equation}%
\nomenclature[a11]{$\Dm$}{or $\Dm(\bp_0)$, the smallest edge length connected to $\bp_0$}%
...
\nomenclature[e03]{$|\bt_v|$}{the number of faces $\bt$ in $N_v$, always $\geq 2$, typically $\leq 12$}%
