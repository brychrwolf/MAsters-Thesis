...
\nomenclature[a01]{$\bp$}{a point in $\bM$}%
\nomenclature[a02]{$\bp_0$}{the center point of $\bN$}%
\nomenclature[a03]{$\bp_i$}{a one-ring neighbor of $\bp_0$}%
\nomenclature[a04]{$\bM$}{a discrete manifold}%
\nomenclature[a05]{$m$}{number of vertices in $\bM$}%
\nomenclature[a06]{$\bN$}{a one-ring neighborhood}%
\nomenclature[a07]{$n$}{the number of points in $\bN$}%
...
\nomenclature[a10]{$\Dm$}{or $\Dm(\bp_0)$, the smallest edge length connected to $\bp_0$}%
%
...
\nomenclature[a09]{$\bs_i$}{the circle sectors which comprise $\bG$}%
...
\end{equation}%
\nomenclature[a11]{$\gDm$}{the smallest edge length in $\bM$}%
...
\begin{algorithm}
	...
	\bigskip
	\FuncSty{function PreCalculate}\FuncArgSty{(a,b)}\;
	\For{$i\leftarrow 1\;\KwTo\;m$}{
		\For{$j\leftarrow 1\;\KwTo\;n_i$}{ %TODO:n_i to nomenclature?
			$\eta^i_j \leftarrow |\bp_j - \bp_i|$\;
		}
		$\Dm^i \leftarrow \min^{n_i}_{j=1}(\eta^i_j)$\;
	}
	...
	\For{$v\leftarrow 1 \KwTo m$}{
		\For{$f\leftarrow 1 \KwTo n$}{
			$\alpha_i\leftarrow stuff$\;
		}
		$\alpha_i\leftarrow stuff$\;
	}
...
\end{algorithm}%
\nomenclature[f01]{$\eta_{i,j}$}{the length of the edge between $\bp_i$ and $\bp_j$}%
