...
set key outside enhanced title "Experiment Code †"

set logscale xy
set ylabel "Compute Time (seconds) - Log Scale"
set xlabel "Fast One-Ring Smothing Filter Convolutions - Log Scale"

plot for [i=3:12] data every ::1::8  using 2:i with linespoints title "MS.".columnhead(i) ls ceil(9-(i-1)*8/10), \
	 for [i=3:12] data every ::9::16 using 2:i with linespoints title "TS.".columnhead(i) ls ceil(9-(i-1)*8/10), \
	 for [i=3:12] data every ::17::24 using 2:i with linespoints title "MP.".columnhead(i) ls ceil(9-(i-1)*8/10), \
	 for [i=3:12] data every ::25::32 using 2:i with linespoints title "TP.".columnhead(i) ls ceil(9-(i-1)*8/10)
