...
\author{Bryan Wolfford}
\title{GUGPU Accelerated Iterative Filtering of Scalar Fields on Discrete Manifolds}
\date{\today} %TODO: Fix date upon submission
...
Serial is slow.~\cite[p.~00]{SourceNeeded}
Concurrency is fast.~\cite[p.~00]{SourceNeeded}
GPUs are commercially available, so use them to exploit concurrency.
~\cite[p.~00]{SourceNeeded}
...
faces (t u , t v ) and (b) the face t w added along the non-manifold edge e k = e m and (c) having a
face t w embedded with a wrong orientation.~\cite[p.~30-31]{Mara12}
...
agglutination face be t 2 ∶= {p c , p b , p a }, we can simplify: s = s 1 = s 2 , A(t) = A(t 1 ) = A(t 2 )
and n̂ 2 = −n̂ 1 and substitute:⎛ s x 1 ⎞⎛ s x 2 ⎞⎛ s x ⎞
...
∃ t i ∨t j ∈ L f with t i ∶= {A i , B i , C i } and t j ∶= {C i , B i , A i }∧{B i , A i , C i }∧{A i , C i , B i } (2.24)
...
Serial vs Multi-threaded vs GPU order from slow to fast.
...
\section{Versions <9, vs >= 9} %TODO: Major Differences in versions
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% State of the Art, Fast 1-Ring Smoothing
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{State of the Art, Fast 1-Ring Smoothing}~\cite[p.~5]{Mara17}
...
Stabdalone vs integration with legacy code.
...
\centering
\subfloat[wireframe]{
  \resizebox{0.48\linewidth}{!}{\input{example_meshes/h.tikz_labels.tex}}
  \label{fig:h.a}}
\subfloat[colored]{
  \includegraphics[width=0.48\linewidth]{example_meshes/h_colored.png}
  \label{fig:h.b}}
